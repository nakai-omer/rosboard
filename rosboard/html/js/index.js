...
if (window.localStorage && window.localStorage.subscriptions) {
  if (
    ...
    window.location.search.indexOf('reset') !== -1
  ) {
    subscriptions = {}
...
let onSystem = function (system) {
  if (system.hostname) {
    ...
    //$('.mdl-layout-title').text('Nakai Robotics')
  }
...
let onTopics = function (topics) {
  ...
  $('<a></a>')
    ...
    .click(() => {
      initSubscribe({
        topicName: '_dmesg',
        topicType: 'rcl_interfaces/msg/Log'
      })
    })
    .text('dmesg')
  ...
  $('<a></a>')
    ...
    .click(() => {
      initSubscribe({
        topicName: '_top',
        topicType: 'rosboard_msgs/msg/ProcessList'
      })
    })
    .text('Processes')
    .appendTo($('#topics-nav-system'))
  ...
  $('<a></a>')
    ...
    .click(() => {
      initSubscribe({
        topicName: '_system_stats',
        topicType: 'rosboard_msgs/msg/SystemStats'
      })
    })
    .text('System stats')
    .appendTo($('#topics-nav-system'))
